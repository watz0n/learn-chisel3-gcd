...
import org.scalatest.exceptions._   // For the "TestFailedException" declaration 
                                    //Ref: https://www.programcreek.com/scala/org.scalatest.exceptions.TestFailedException

class GCDBehavPeekPokeTester(dut: gcd_behav, a:Int, b:Int, c:Int) extends PeekPokeTester(dut)  {
...
class GCDBehavPeekPokeSpec extends ChiselFlatSpec with Matchers {
  
  //Use scalatest FlatSpec style, build-in ChiselFlatSpec
  //Ref: http://www.scalatest.org/at_a_glance/FlatSpec

  it should "Test1: gcd_behav should be elaborate normally" in {
    elaborate { 
      new gcd_behav 
    }
  ...
  it should "Test2: GCDBehav Tester return the correct result" in {
    ...
    gcd_tests.foreach { tupleElement => {
      ...
      try {
        chisel3.iotesters.Driver.execute(() => new gcd_behav, manager) {
          dut => new GCDBehavPeekPokeTester(dut, a, b, c)
        } should be (true)
      } catch {
