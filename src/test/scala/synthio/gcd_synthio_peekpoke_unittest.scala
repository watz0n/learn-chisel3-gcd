...
class GCDSynthIOPeekPokeTester(dut: gcd_synthio, a:Int, b:Int, c:Int) extends PeekPokeTester(dut)  {
...
class GCDSynthIOPeekPokeSpec extends ChiselFlatSpec with Matchers {
  
  //Use scalatest FlatSpec style, build-in ChiselFlatSpec
  //Ref: http://www.scalatest.org/at_a_glance/FlatSpec

  it should "Test1: gcd_synthio should be elaborate normally" in {
    elaborate { 
      new gcd_synthio
    }
  ...
  it should "Test2: GCDSynthIO Tester return the correct result" in {
    ...
    gcd_tests.foreach { tupleElement => {
      ...
      try {
        chisel3.iotesters.Driver.execute(() => new gcd_synthio, manager) {
          dut => new GCDSynthIOPeekPokeTester(dut, a, b, c)
        } should be (true)
      } catch {
